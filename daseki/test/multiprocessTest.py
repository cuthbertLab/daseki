# -*- coding: utf-8 -*-
# ------------------------------------------------------------------------------
# Name:         multiprocessTest.py
# Purpose:      Controller for all tests in daseki run concurrently.
#
# Authors:      Michael Scott Cuthbert
#
# Copyright:    Copyright © 2012-16 Michael Scott Cuthbert / cuthbertLab
# License:      BSD, see license.txt
# ------------------------------------------------------------------------------

'''
Multiprocess testing.  Tests all doctests and Test unittest objects in all
modules that are imported when running 'import daseki'.  Runs threads on
...
def runOneModuleWithoutImp(args):
    ...
    if moduleObject == 'skip':
        ...
        success = ('%s is in the daseki directory but not imported in daseki. Skipped -- fix!'
    ...
    try:
        ...
        # print('running Tests...\n')
        runner = commonTest.DasekiTestRunner(verbosity=verbosity)
        try:
            ...
            print(f'*** Exception in running {moduleName}: {excp}...\n')
            return ModuleResponse('TrappedException', fp, moduleName, None, str(excp))
    except Exception as excp:  # pylint: disable=broad-except
        print(f'*** Large Exception in running {fp}: {excp}...\n')
...
def mainPoolRunner(testGroup=('test',), restoreEnvironmentDefaults=False, leaveOut=1):
    ...
    modGather = commonTest.ModuleGather(useExtended=True, autoWalk=False)
    modGather.moduleSkip.append('common/parallel.py') # cannot test parallel...
    modGather.walk()
    ...
    pool = multiprocessing.Pool(processes=poolSize)  # pylint: disable=not-callable
    ...
    while continueIt is True:
        try:
                ...
                if rt is not None:
                    ...
                    if not newResult.errors and not newResult.failures:
                        ...
                        print(
                            ...
                            f'{len(newResult.errors)} errors {len(newResult.failures)} '
                            f'failures in {rt} secs'
                        )
            timeouts = 0
            eventsProcessed += 1
            ...
            eventsProcessed += 1
            exceptionLog = ModuleResponse('UntrappedException', None, f'{excp}')
...
def printSummary(summaryOutput, timeStart, pathsToRun):
    ...
    for moduleResponse in summaryOutput:
        ...
        if moduleResponse.returnCode == 'Skipped':
            ...
            otherSummary.append(
                f'Trapped Exception for module {moduleResponse.moduleName}, at {moduleResponse.fp}: {moduleResponse.testRunner}')
            ...
            if moduleResponse.success:
                successSummary.append(
                    ...
                    f'{moduleResponse.testsRun:d} tests in {moduleResponse.runTime:d} seconds'
                )
            else:
                ...
                for e in errorsList:
                    outStr += e + '\n'
                    errorsFoundSummary.append(f'{e}')
                for f in failuresList:
                    outStr += f + '\n'
                    errorsFoundSummary.append(f'{f}')
                # for e in errorsList:
                #     print e[0], e[1]
    ...
    outStr += f'Ran {totalTests:d} tests in {elapsedTime:.4f} seconds\n'
    sys.stdout.flush()
    print(outStr)
    sys.stdout.flush()

    import datetime
    lastResults = os.path.join(common.getDefaultRootTempDir(), 'lastResults.txt')
...
if __name__ == '__main__':
    ...
    # mm = mg.getModuleWithoutImp('daseki.retro.play')
    # print(mm)
    mainPoolRunner()
