...
def runOneModuleWithoutImp(args):
    ...
    if moduleObject == 'skip':
        ...
        success = '%s is in the daseki directory but not imported in daseki. Skipped -- fix!' % modGath._getNamePeriod(fp)
        #print(success)
    ...
    try:
        ...
        try:
            ...
            print('*** Exception in running %s: %s...\n' % (moduleName, excp))
            return ModuleResponse("TrappedException", fp, moduleName, None, str(excp))
    except Exception as excp: # pylint: disable=broad-except
        print('*** Large Exception in running %s: %s...\n' % (fp, excp))
