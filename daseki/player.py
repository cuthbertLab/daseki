...
_positionNames = (
    'unknown pitcher catcher firstbase secondbase '
...
class PlayerYear(common.ParentMixin):
    ...
    def __init__(self, _playerId, *, parent=None):
...
class PlayerGame(common.ParentMixin):
    ...
    def __repr__(self):
        return '<%s.%s %s,%s: %s (%s):%s>' % (self.__module__,
    ...
    def plateAppearancesAsRunner(self, searchBefore=True, searchAfter=True, searchScoring=True):
        ...
        Check against https://www.baseball-reference.com/boxes/SDN/SDN201304090.shtml
    ...
    def boxScoreStatline(self,
                         fields='atBats runs hits walks strikeOuts',
                         paddingInfo=None):
        ...
        if isinstance(fields, str):
            fields = fields.split()
        ll = (self.lastPlusInitial()
              + ' '
              + ','.join([positionAbbrevs[p] for p in self.positions])
              )
        if self.isSub:
            ll = (' ' * pi['subIndent']) + ll
        ll = ll.ljust(pi['nameSpace'])
        for f in fields:
            ll += str(getattr(self, f)).rjust(pi['fieldSpace'])
        return ll
...
class LineupCard(common.ParentMixin):
    '''
    Represents all the players for a given team for a given game.
    ...
    def __init__(self, visitOrHome, *, parent=None):
        ...
        # batting order 0 will always be None
        self.playersByBattingOrder: list[list['PlayerGame']] = [[] for _ in range(10)]
    ...
    def __repr__(self):
        ...
        return f'<{self.__module__}.{self.__class__.__name__} ' \
               f'{visitorNames[self.visitOrHome]} ({gi})>'
    ...
    def playsWithSubstitutions(self, startNumber=0, endNumber=99999):
        ...
        '''
        return [r.playNumber for r in self.lineupData
                if startNumber <= r.playNumber <= endNumber]
    ...
    def multipleSubs(self, startNumber=0, endNumber=99999):
        '''
        find all the times in which multiple substitutions
        happened in the same play sequence.
    ...
    def battingOrderAtPlayNumber(self, playNumber=0):
        ...
        batters = [None for _ in range(10)]
    ...
    def __init__(self, playerId, playerName, visitOrHome, battingOrder, position, *, parent=None):
        ...
        try:
            ...
            raise RetrosheetException(
                f'Parse error for player {playerName}: {ve}'
            )
