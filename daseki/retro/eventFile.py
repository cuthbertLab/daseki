...
from ..exceptionsDS import RetrosheetException
...
class EventFile(object):
    ...
    The other attribute is the "startComments" list which is a
    list of Comment objects that
    describe the .EVN or .EVA file but not any particular
    game (such as the encoder's name).
    ...
    And these files have a bunch of :class:`daseki.retro.protoGame.ProtoGame`
    objects, one for every home game, usually 81.
    ...
    def __repr__(self):
        ...
        return f'<{self.__module__}.{self.__class__.__name__} {stripName}>'
            ...
            if currentProtoGame is None:
                if eventType != 'com':
                    raise RetrosheetException(
                        f'Found a non-comment before id: {eventLine!r}'
                    )
