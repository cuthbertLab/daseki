...
class PlateAppearance(common.ParentMixin):
    ...
    def __getattr__(self, attr):
        ...
        if le is None:
            raise IndexError(
                f'{self.__class__.__name__!r} object has events to search ' 
                'for attributes on'
            )
        if hasattr(le, attr):
    ...
    def __repr__(self):
        ...
        return (f'<{self.__module__}.{self.__class__.__name__} '
                f'{self.inningNumber}-{self.plateAppearanceInInning}{incomplete}: '
                f'{self.batterId}:{self.lastPlayEvent.raw}>')
